...
class FACTRTeleop(ABC):
    ...
    def run(self):
        """Starts the main control loop. Blocks until interrupted."""
        self._running = True
        logger.info(f"FACTR TELEOP {self.name}: Control loop started at {1/self.dt:.0f} Hz")
        try:
            while self._running:
    ...
    def _prepare_dynamixel(self):
        ...
        try:
            ...
            logger.error(f"Port {self.dynamixel_port} not found. Please check the connection.")
            return
    ...
    def _prepare_inverse_dynamics(self):
        ...
        urdf_model_path = os.path.join(urdf_dir, self.config["arm_teleop"]["leader_urdf"])
        self.pin_model, _, _ = pin.buildModelsFromUrdf(filename=urdf_model_path, package_dirs=urdf_dir)
    ...
    def _get_dynamixel_offsets(self, verbose=True):
        ...
        for i in range(self.num_arm_joints):
            best_offset = 0
            best_error = 1e9
            # intervals of pi/8 (finer grid to reduce rounding error, max ~5.6°)
            for offset in np.linspace(-20 * np.pi, 20 * np.pi, 20 * 16 + 1):
    ...
    def _match_start_pos(self):
        ...
        while (np.linalg.norm(curr_pos - self.initial_match_joint_pos[0:self.num_arm_joints]) > 0.8):
            ...
            logger.info(
                f"FACTR TELEOP {self.name}: Please match starting joint pos. Current error: {current_joint_error:.3f}"
            )
            curr_pos, _, _, _ = self.get_leader_joint_states()
            time.sleep(0.5)
        logger.info(f"FACTR TELEOP {self.name}: Initial joint position matched.")
